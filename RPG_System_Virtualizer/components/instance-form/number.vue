...
export default {
  ...
  computed: {
    ...
    func () {
      ...
      let res = mathjs.evaluate(this.$store.getters.getNewInstanceProperty(this.data.id).trim())
      const roundAt = this.$store.getters['equation-rounder/get'](this.funcObj.id, 'functionId').value
      const decimal = res % 1
      if (decimal !== 0) { decimal > parseFloat(roundAt) ? res = mathjs.ceil(res) : res = mathjs.floor(res) }
      return res
    }
  },
  watch: {
